// sorular/tarih-23.js - XX. YÜZYIL BAŞLARINDA DÜNYA
window.tarih_23_sorulari = [
{
    "question": "1823’te ortaya konulan ve yalnızlık politikası olarak anılan ABD dış politikasının mimarı aşağıdakilerden hangisidir?",
    "options": ["A) Woodrow Wilson", "B) Franklin D. Roosevelt", "C) James Monroe", "D) Theodore Roosevelt", "E) Harry Truman"],
    "correctAnswerIndex": 2,
    "explanation": "Monroe Doktrini (1823) Başkan James Monroe’ya aittir. Wilson On Dört Nokta ile karıştırılmamalıdır."
},
{
    "question": "Monroe Doktrini’ne göre ABD’nin temel yaklaşımı aşağıdakilerden hangisidir?",
    "options": [
        "A) Avrupa ittifaklarına asker göndermek",
        "B) Japonya’ya savaş açmak",
        "C) Osmanlı topraklarını paylaşmak",
        "D) Milletler Cemiyeti’ni kurmak",
        "E) Kendi kıtasına Avrupa saldırısı olmadığı sürece Avrupa’nın iç işlerine karışmamak"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Yalnızlık politikası: ABD kıtasına saldırı yoksa Avrupa iç işlerine karışmaz."
},
{
    "question": "1904–1905 Rus–Japon Savaşı’nın sonucu aşağıdakilerden hangisidir?",
    "options": ["A) Rusya kesin zafer kazandı", "B) Çin kazandı", "C) Savaş berabere bitti", "D) Japonya kazandı", "E) ABD aracılığıyla Rusya galip ilan edildi"],
    "correctAnswerIndex": 3,
    "explanation": "Savaşı Japonya kazandı; Çarlık sarsıldı."
},
{
    "question": "Kanlı Pazar’ın yaşandığı ve Rus Duma Meclisi’nin açıldığı yıl aşağıdakilerden hangisidir?",
    "options": ["A) 1904", "B) 1905", "C) 1915", "D) 1917", "E) 1918"],
    "correctAnswerIndex": 1,
    "explanation": "Kanlı Pazar ve Duma 1905’tir. 1917 Bolşevik İhtilali’dir."
},
{
    "question": "1917 Bolşevik İhtilali’nde Lenin’in kitlelere verdiği üçlü vaat hangisidir?",
    "options": ["A) Barış, ekmek ve toprak", "B) Hürriyet, eşitlik, kardeşlik", "C) Din, dil, ırk", "D) Sanayi, tarım, ticaret", "E) Ordu, donanma, hava"],
    "correctAnswerIndex": 0,
    "explanation": "Lenin: barış, ekmek ve toprak."
},
{
    "question": "Rusya’nın I. Dünya Savaşı’ndan çekildiği antlaşma aşağıdakilerden hangisidir?",
    "options": ["A) Versailles", "B) Sevr", "C) Brest-Litovsk", "D) Locarno", "E) Rapallo"],
    "correctAnswerIndex": 2,
    "explanation": "1918 Brest-Litovsk Antlaşması ile Rusya savaştan çekildi."
},
{
    "question": "1918–1921 yıllarında Rusya’da Kızıl Ordu’nun karşısında yer alan güç aşağıdakilerden hangisidir?",
    "options": ["A) Kızıl Ordu", "B) Basmacılar", "C) Yeşil Ordu", "D) Beyaz Ordu", "E) Korbaşılar"],
    "correctAnswerIndex": 3,
    "explanation": "İç savaşta Beyaz Ordu–Kızıl Ordu çatışması yaklaşık üç yıl sürdü."
},
{
    "question": "1921 NEP uygulamasının özelliği aşağıdakilerden hangisidir?",
    "options": [
        "A) Tüm özel mülkiyetin derhal kaldırılması",
        "B) Tarım ürünlerine el konulmasına devam",
        "C) Özel mülkiyete dokunulmaması ve piyasanın kısmen geri gelmesi",
        "D) Kolektif çiftliklerin zorunlu kılınması",
        "E) Beş yıllık planın başlaması"
    ],
    "correctAnswerIndex": 2,
    "explanation": "NEP’te tarıma el koymadan vazgeçildi, özel mülkiyete dokunulmadı, nepmen ve piyasa geri geldi. Kolektifleştirme Stalin dönemidir."
},
{
    "question": "NEP döneminde ticari işletmelerin bırakıldığı kişilere verilen ad aşağıdakilerden hangisidir?",
    "options": ["A) Nepmen", "B) Kulak", "C) Korbaşı", "D) Şogun", "E) Kolhoz"],
    "correctAnswerIndex": 0,
    "explanation": "Ticari işletmeler nepmenlere bırakıldı."
},
{
    "question": "Sınıfsız ortak mülkiyete dayalı komünist ideolojinin kurucuları aşağıdakilerin hangisinde doğru verilmiştir?",
    "options": ["A) Lenin – Stalin", "B) Marx – Engels", "C) Troçki – Lenin", "D) Mao – Ho Chi Minh", "E) Castro – Guevara"],
    "correctAnswerIndex": 1,
    "explanation": "Kurucular Karl Marx ve Friedrich Engels’tir."
},
{
    "question": "SSCB’nin federasyon olarak kurulması ve Stalin’in başa geçmesi yılları hangi seçenekte doğru verilmiştir?",
    "options": ["A) 1917 – 1918", "B) 1921 – 1922", "C) 1922–1923 ve 1924", "D) 1924 – 1929", "E) 1929 – 1936"],
    "correctAnswerIndex": 2,
    "explanation": "Ülke 1922–1923’te SSCB adını aldı; 1924’te Stalin başa geçti."
},
{
    "question": "Stalin döneminde köylü topraklarının birleştirilmesine verilen ad aşağıdakilerden hangisidir?",
    "options": ["A) NEP", "B) Kliring", "C) Kolektifleştirme", "D) Meiji", "E) Mandaterlik"],
    "correctAnswerIndex": 2,
    "explanation": "Küçük topraklar alınıp birleştirildi: kolektifleştirme. NEP bunun tersi yönde esnekliktir."
},
{
    "question": "Rusya Müslümanları I. Kongresi’nde öne çıkan isimler aşağıdakilerin hangisinde birlikte verilmiştir?",
    "options": [
        "A) Yusuf Akçura – İsmail Gaspıralı",
        "B) Enver Paşa – Şeyh Şamil",
        "C) Mehmet Emin Resulzade – Picasso",
        "D) Şerif Hüseyin – İbn Suud",
        "E) Fleming – Steinbeck"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Kongrede Yusuf Akçura ve İsmail Gaspıralı öne çıkar."
},
{
    "question": "Ufa Müslümanlarının Duma temsilcisi olan, Basmacı hareketine katılan ve 1925’te Türkiye’ye gelerek İstanbul Üniversitesi’nde görev yapan kişi kimdir?",
    "options": ["A) Yusuf Akçura", "B) İsmail Gaspıralı", "C) Ahmet Zeki Velidi Togan", "D) Mehmet Emin Resulzade", "E) Enver Paşa"],
    "correctAnswerIndex": 2,
    "explanation": "Ahmet Zeki Velidi Togan Duma temsilcisi, Basmacı’ya katıldı, 1925’te Türkiye’ye geldi."
},
{
    "question": "Basmacı sözcüğünün anlamı ve hareketin başladığı kent hangi seçenekte doğru verilmiştir?",
    "options": [
        "A) Göçmen – Bakü",
        "B) Öğrenci – Ufa",
        "C) Çiftçi – Semerkant",
        "D) Tüccar – Buhara",
        "E) Baskın yapan, hücum eden – Hokand"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Basmacı: baskın yapan. Hareket Hokand’da başladı."
},
{
    "question": "Basmacı hareketine katılmasıyla mücadelenin şiddetlendiği ve 1922’de şehit düşen Osmanlı paşası kimdir?",
    "options": ["A) Mustafa Kemal", "B) Kazım Karabekir", "C) Enver Paşa", "D) Cemal Paşa", "E) Ali Fuat Cebesoy"],
    "correctAnswerIndex": 2,
    "explanation": "Enver Paşa Basmacı’ya katıldı, 1922’de şehit oldu. Hareket 1936’ya kadar sürdü."
},
{
    "question": "28 Mayıs 1918’de Azerbaycan Halk Cumhuriyeti’ni kuran lider ve bu cumhuriyeti resmi tanıyan ilk devlet hangi seçenektedir?",
    "options": [
        "A) Stalin – SSCB",
        "B) Mehmet Emin Resulzade – Osmanlı Devleti",
        "C) Lenin – İngiltere",
        "D) Şerif Hüseyin – Fransa",
        "E) Gaspıralı – ABD"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Resulzade kurdu; Türk tarihinin cumhuriyetle yönetilen ilk devleti. İlk tanıyan Osmanlı’dır."
},
{
    "question": "Şeyh Şamil’in Kafkasya’daki direnişine Rusların ve Müslümanların verdiği adlar hangisidir?",
    "options": [
        "A) Ruslar gazavat, Müslümanlar müridizm",
        "B) Ruslar müridizm, Müslümanlar gazavat",
        "C) Her iki taraf Basmacı",
        "D) Her iki taraf NEP",
        "E) Ruslar Meiji, Müslümanlar Monroe"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Ruslar müridizm, Müslümanlar gazavat der. Şeyh Şamil Kafkas Kartalı’dır."
},
{
    "question": "San Remo Konferansı’na göre Fransa ve İngiltere’nin Ortadoğu mandaları hangi seçenekte doğru eşleştirilmiştir?",
    "options": [
        "A) Fransa: Irak–Filistin / İngiltere: Suriye–Lübnan",
        "B) İkisi de yalnızca Filistin",
        "C) Fransa: Mısır–Sudan / İngiltere: Suriye",
        "D) Fransa: Suudi Arabistan / İngiltere: İran",
        "E) Fransa: Suriye–Lübnan / İngiltere: Irak–Filistin–Ürdün"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Fransa Suriye–Lübnan; İngiltere Irak–Filistin–Ürdün."
},
{
    "question": "Şerif Hüseyin’in oğullarından Abdullah ve Faysal’ın atandığı yerler hangisidir?",
    "options": ["A) Abdullah Ürdün, Faysal Irak", "B) Abdullah Irak, Faysal Ürdün", "C) İkisi de Hicaz", "D) Abdullah Suriye, Faysal Lübnan", "E) Abdullah Filistin, Faysal Mısır"],
    "correctAnswerIndex": 0,
    "explanation": "Abdullah Ürdün’e, Faysal Irak’a tayin edildi."
},
{
    "question": "1936’da Suudi Arabistan’da petrol imtiyazı verilen şirket ve bunun sonucu aşağıdakilerden hangisidir?",
    "options": [
        "A) ARAMCO – ABD etkinliği bölgede arttı",
        "B) BP – İngiltere etkisi azaldı",
        "C) Shell – Japonya güçlendi",
        "D) Total – Fransa çekildi",
        "E) Gazprom – SSCB petrolü kesti"
    ],
    "correctAnswerIndex": 0,
    "explanation": "1932 Suudi Arabistan Krallığı; 1936 ARAMCO imtiyazı ABD etkisini artırdı."
},
{
    "question": "Filistin sorununun başlangıcı sayılan 1917 belgesi aşağıdakilerden hangisidir?",
    "options": ["A) Monroe Doktrini", "B) Balfour Deklarasyonu", "C) Briand–Kellogg Paktı", "D) Locarno", "E) NEP"],
    "correctAnswerIndex": 1,
    "explanation": "1917 Balfour Deklarasyonu Filistin sorununun başlangıcıdır."
},
{
    "question": "Fransa’nın Anadolu’nun güneyinden çekilmesini sağlayan antlaşma aşağıdakilerden hangisidir?",
    "options": ["A) Sevr", "B) Mondros", "C) Lozan", "D) Ankara Antlaşması", "E) San Remo"],
    "correctAnswerIndex": 3,
    "explanation": "Fransa Ankara Antlaşması ile güneyden çekildi. 1926 Lübnan, 1930 Suriye bağımsızlık süreçleri."
},
{
    "question": "Japonya’da derebeylikleri kaldırıp merkezi devlet kuran ve Meiji Restorasyonu’nu yürüten imparator kimdir?",
    "options": ["A) Hirohito", "B) Şogun Iwakura", "C) Tokugawa", "D) Mutsuhito", "E) Mao"],
    "correctAnswerIndex": 3,
    "explanation": "1867’de tahta geçen Mutsuhito (Meiji) derebeyliği kaldırdı. Şogun ordu komutanıdır, imparator değildir."
},
{
    "question": "Meiji Restorasyonu’nda anayasa ve donanma için örnek alınan ülkeler hangi seçenekte doğru verilmiştir?",
    "options": [
        "A) Anayasa: ABD / Donanma: Rusya",
        "B) Anayasa: İngiltere / Donanma: Prusya",
        "C) Anayasa: Osmanlı / Donanma: Çin",
        "D) Anayasa: Fransa / Donanma: İtalya",
        "E) Anayasa: Prusya / Donanma: İngiltere"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Prusya tipi anayasa; İngiliz donanması örnek alındı. Prusya’dan uzman getirildi."
},
{
    "question": "ABD’de 1920’lerde üretim ve istihdamın yükseldiği döneme verilen ad aşağıdakilerden hangisidir?",
    "options": ["A) Kara Perşembe", "B) Meiji", "C) New Deal", "D) NEP", "E) Kükreyen Yirmiler"],
    "correctAnswerIndex": 4,
    "explanation": "Bu yıllara Kükreyen Yirmiler denir. Kara Perşembe 1929 çöküşüdür."
},
{
    "question": "Tarihe Kara Perşembe olarak geçen borsa çöküşünün tarihi aşağıdakilerden hangisidir?",
    "options": ["A) 3 Ekim 1928", "B) 24 Ekim 1929", "C) 29 Ekim 1918", "D) 10 Ocak 1920", "E) 28 Mayıs 1918"],
    "correctAnswerIndex": 1,
    "explanation": "3 Ekim 1929’da düşüş başladı; 24 Ekim 1929 Perşembe dibe vurdu. 4,2 milyar dolar yok oldu."
},
{
    "question": "1929 krizine karşı Türkiye’nin uyguladığı “malımı sattığım devletten mal alırım” sistemi aşağıdakilerden hangisidir?",
    "options": ["A) NEP", "B) Kapitülasyon", "C) Kliring", "D) Düyun-u Umumiye", "E) ARAMCO"],
    "correctAnswerIndex": 2,
    "explanation": "Kliring (takas) sistemi. Ayrıca devletçilik, gümrük, yerli malı ve Milli İktisat ve Tasarruf Cemiyeti."
},
{
    "question": "Aşağıdakilerden hangisi 1929 krizine karşı Türkiye’nin aldığı önlemlerden biri değildir?",
    "options": [
        "A) Korumacı ve devletçi politikalara yönelmek",
        "B) Gümrük vergilerini yükseltmek",
        "C) Yerli malını teşvik etmek",
        "D) Milli İktisat ve Tasarruf Cemiyeti kurmak",
        "E) Kapitülasyonları yeniden yürürlüğe koymak"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Kapitülasyonlar Lozan’da kalkmıştır; kriz tedbiri değildir."
},
{
    "question": "Milletler Cemiyeti’nin kuruluş tarihi ve merkezi hangi seçenekte doğru verilmiştir?",
    "options": ["A) 1918 – Paris", "B) 10 Ocak 1920 – Cenevre", "C) 1925 – Locarno", "D) 1928 – Washington", "E) 1929 – New York"],
    "correctAnswerIndex": 1,
    "explanation": "10 Ocak 1920, merkez Cenevre (Cemiyet-i Akvam). Türkiye sonradan üyedir."
},
{
    "question": "Locarno Antlaşması (1925) ile ilgili hangisi doğrudur?",
    "options": [
        "A) Türkiye imzacıdır",
        "B) Almanya dışlanmaktan çıkarılarak uluslararası iş birliğine alındı",
        "C) I. Dünya Savaşı’nı başlattı",
        "D) SSCB’yi kurdu",
        "E) Filistin’i Osmanlı’ya iade etti"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Türkiye Locarno’da yoktur. Almanya kısa sonra Milletler Cemiyeti’ne üye oldu."
},
{
    "question": "1928 Briand–Kellogg Paktı’na göre hangisi doğrudur?",
    "options": [
        "A) Savunmaya dayanmayan savaş kanun dışı sayılmıştır; Türkiye aynı yıl katılmıştır",
        "B) Türkiye pakta dahil olmamıştır",
        "C) Yalnızca Almanya imzalamıştır",
        "D) Monroe Doktrini’ni iptal etmiştir",
        "E) Basmacı hareketini desteklemiştir"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Paris 1928; aynı yıl SSCB ve Türkiye dahil oldu. Saldırı savaşı yasaklandı."
},
{
    "question": "1937’de Guernica kasabasının bombalanmasını tablosuna aktaran kübizm öncüsü kimdir?",
    "options": ["A) Charlie Chaplin", "B) Alexander Fleming", "C) John Steinbeck", "D) Pablo Picasso", "E) Adnan Saygun"],
    "correctAnswerIndex": 3,
    "explanation": "Picasso’nun Guernica tablosu 1937’dir."
},
{
    "question": "Penisilini bulan bilim insanı aşağıdakilerden hangisidir?",
    "options": ["A) Louis Pasteur", "B) Edison", "C) Marie Curie", "D) Einstein", "E) Alexander Fleming"],
    "correctAnswerIndex": 4,
    "explanation": "Alexander Fleming penisilini buldu. Pasteur tuzak şıktır."
},
{
    "question": "Gazap Üzümleri adlı eserin yazarı kimdir?",
    "options": ["A) Picasso", "B) Marx", "C) Charlie Chaplin", "D) John Steinbeck", "E) Halide Edip"],
    "correctAnswerIndex": 3,
    "explanation": "John Steinbeck — Gazap Üzümleri."
},
{
    "question": "Şarlo karakteriyle tanınan, Modern Zamanlar ve Diktatör filmlerini çeken sanatçı kimdir?",
    "options": ["A) Pablo Picasso", "B) Mutsuhito", "C) John Steinbeck", "D) Charlie Chaplin", "E) Wilson"],
    "correctAnswerIndex": 3,
    "explanation": "Charlie Chaplin (Şarlo): Modern Zamanlar ve Diktatör."
}
];
