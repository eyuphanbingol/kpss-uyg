// sorular/cografya-27.js — COĞRAFYA GENEL TEKRAR (1–26 kapsamlı)
window.cografya_27_sorulari = [
    {
        "question": "Türkiye’nin matematik konumu için hangisi doğrudur?",
        "options": [
            "A) 36°–42° K enlemleri ile 26°–45° D boylamları arasındadır",
            "B) 20°–36° K enlemleri arasındadır",
            "C) Tamamı Güney Yarımküre’dedir",
            "D) Boylam aralığı 10°–20° D’dir",
            "E) Ekvator üzerinde yer alır"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Klasik KPSS bilgisi: 36–42 K / 26–45 D."
    },
    {
        "question": "Aynı enlemdeki iki merkezde sıcaklıklar belirgin farklıysa temel neden genelde hangisidir?",
        "options": [
            "A) Matematik konum",
            "B) Özel konum (yükselti, denizellik, bakı vb.)",
            "C) Boylam farkı",
            "D) Saat dilimi",
            "E) Jeoid şekli"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Aynı enlem → enlem etkisi aynıdır; fark özel konumdan gelir."
    },
    {
        "question": "Kuzey–güney doğrultusunda sıcaklık farkı öncelikle hangi faktörle açıklanır?",
        "options": [
            "A) Boylam",
            "B) Enlem",
            "C) Bakı",
            "D) Denizellik",
            "E) Kıtasal konum"
        ],
        "correctAnswerIndex": 1,
        "explanation": "K–G kıyas = enlem."
    },
    {
        "question": "Karadeniz Dağları’nda kışın kuzey yamaçların güney yamaçlardan daha ılık olması hangisiyle bağdaşır?",
        "options": [
            "A) Klasik bakı kuralının bozulması (ters olay / nem–bulut)",
            "B) Enlemin artması",
            "C) Boylamın azalması",
            "D) Fiyort oluşumu",
            "E) Delta oluşumu"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Karadeniz’de bakı–sıcaklık ilişkisi klasik kurala uymayabilir (ters olay)."
    },
    {
        "question": "İki merkez arasındaki yerel saat farkı nasıl bulunur?",
        "options": [
            "A) Enlem farkı × 4 dakika",
            "B) Boylam farkı × 4 dakika",
            "C) Yükselti farkı × 4",
            "D) Uzaklık (km) ÷ 4",
            "E) Boylam farkı × 15 dakika"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ardışık boylamlar ≈ 4 dk; fark = boylam farkı × 4."
    },
    {
        "question": "Aynı enlemde İzmir’in Van’dan daha sıcak olmasının temel nedeni hangisidir?",
        "options": [
            "A) Boylam",
            "B) Yükselti",
            "C) Enlem",
            "D) Fay hatları",
            "E) Karstik yapı"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Van daha yüksektir → özel konum (yükselti)."
    },
    {
        "question": "Yazın Şanlıurfa’nın aynı enlemdeki Antalya’dan daha sıcak olması öncelikle neye bağlanır?",
        "options": [
            "A) Enlem",
            "B) Karasallık",
            "C) Bakı",
            "D) Buzullaşma",
            "E) Ria kıyı"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Karasal iç kesim yazın daha çok ısınır."
    },
    {
        "question": "Kırılma sonucu yüksekte kalan yerlere ne ad verilir?",
        "options": [
            "A) Graben",
            "B) Horst",
            "C) Delta",
            "D) Morén",
            "E) Obruk"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Horst = yüksekte kalan; graben = çöküntü."
    },
    {
        "question": "Horst–graben sistemi ülkemizde en yaygın olarak nerede görülür?",
        "options": [
            "A) Doğu Karadeniz",
            "B) Batı Anadolu (Ege)",
            "C) İç Anadolu ovaları",
            "D) Trakya platosu",
            "E) GAP ovaları"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ege kırıklı yapı / BAF."
    },
    {
        "question": "Kuzey Anadolu Fayı (KAF) ile Doğu Anadolu Fayı (DAF) hangi merkez çevresinde birleşir?",
        "options": [
            "A) İstanbul",
            "B) Bingöl–Karlıova",
            "C) Antalya",
            "D) İzmir",
            "E) Ankara"
        ],
        "correctAnswerIndex": 1,
        "explanation": "KAF ve DAF Karlıova’da kesişir."
    },
    {
        "question": "Türkiye’de en yaygın dış kuvvet hangisidir?",
        "options": [
            "A) Buzul",
            "B) Rüzgâr",
            "C) Akarsu",
            "D) Dalga",
            "E) Karst"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Akarsular Türkiye’de en etkili dış kuvvettir."
    },
    {
        "question": "Kimyasal çözünmenin en etkili olduğu yerler genelde hangisidir?",
        "options": [
            "A) Kurak ve soğuk yüksek yaylalar",
            "B) Sıcak–nemli alanlar",
            "C) Buzul zirveleri",
            "D) Çöl çekirdekleri",
            "E) Kutuplar"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sıcak + nem → kimyasal çözünme hızlanır."
    },
    {
        "question": "Türkiye’de platoların geniş yer kaplaması hangi sürecin kanıtı olarak gösterilir?",
        "options": [
            "A) Yalnızca buzullaşma",
            "B) Kuvaterner’de toptan yükselme (epirojenez)",
            "C) Delta birikimi",
            "D) Lagün oluşumu",
            "E) Fiyort oluşumu"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Plato yaygınlığı = toptan yükselme kanıtı."
    },
    {
        "question": "Türkiye’nin en yüksek platoları hangi kökene aittir?",
        "options": [
            "A) Karstik (Taşeli)",
            "B) Volkanik (Erzurum–Kars çevresi)",
            "C) Aşınım (Çatalca–Kocaeli)",
            "D) Delta",
            "E) Dalga aşındırması"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Erzurum–Kars volkanik platoları en yüksektir."
    },
    {
        "question": "Delta ovası oluşabilmesi için hangisi gerekli değildir?",
        "options": [
            "A) Bol alüvyon",
            "B) Sakin deniz / gelgit azlığı",
            "C) Kıyıda dik falez",
            "D) Akarsuyun denize dökülmesi",
            "E) Biriktirme alanı"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Falez varsa delta oluşmaz; notlardaki klasik kural."
    },
    {
        "question": "Tektonik (çöküntü) ovalarına örnek hangisidir?",
        "options": [
            "A) Çarşamba Deltası",
            "B) Gediz / Büyük Menderes graben ovaları",
            "C) Bafra Deltası",
            "D) Dalga birikim seti",
            "E) Morén ovası"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ege graben ovaları tektonik kökenlidir."
    },
    {
        "question": "Türkiye akarsularının genel özelliği hangisidir?",
        "options": [
            "A) Rejimleri düzenlidir",
            "B) Engebenin fazla olması nedeniyle hidroelektrik potansiyelleri yüksektir",
            "C) Hepsi denize dökülür",
            "D) Hiçbiri baraj yapılamaz",
            "E) Tümü boyuna (kıyıya paralel) akar"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Eğim/engebe → HES potansiyeli."
    },
    {
        "question": "Akarsu aşındırmasıyla oluşan “V” şekilli vadi için hangisi doğrudur?",
        "options": [
            "A) Olgunluk döneminde yaygındır",
            "B) Gençlik döneminde dik yamaçlıdır",
            "C) Yalnızca buzul vadisidir",
            "D) U şeklindedir",
            "E) Karstiktir"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Genç vadi = V; buzul = U."
    },
    {
        "question": "Türkiye’de rüzgâr aşındırma–biriktirme şekillerinin daha belirgin olduğu yerler hangisidir?",
        "options": [
            "A) Doğu Karadeniz kıyısı",
            "B) Kurak–yarı kurak iç bölgeler",
            "C) Akdeniz’in her yeri",
            "D) Marmara’nın tamamı",
            "E) Buzul zirveleri"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Bitki örtüsü seyrek + kuraklık → rüzgâr etkili."
    },
    {
        "question": "Barkan (hilal biçimli kumulu) için hangisi doğrudur?",
        "options": [
            "A) Nemli ormanlarda oluşur",
            "B) Kurak alanlarda rüzgâr biriktirmesiyle oluşur",
            "C) Buzul aşındırmasıdır",
            "D) Dalga aşındırmasıdır",
            "E) Karstiktir"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Barkan = çöl/kurak kumulu."
    },
    {
        "question": "Türkiye’de deniz seviyesinde buzul şekillerinin görülmemesinin temel nedeni hangisidir?",
        "options": [
            "A) Özel konum",
            "B) Orta Kuşak’ta (matematik konum) yer alması",
            "C) Fay hatları",
            "D) GAP",
            "E) Boylam"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Orta Kuşak → deniz seviyesinde buzul yok; yükseklerde özel konumla var."
    },
    {
        "question": "Sirk gölü nasıl oluşur?",
        "options": [
            "A) Delta birikimiyle",
            "B) Buzul aşındırmasıyla oluşan çukurun suyla dolmasıyla",
            "C) Lagün kapanmasıyla",
            "D) Obruk çökmesiyle",
            "E) Volkanik kraterle her zaman"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sirk → buzul çukuru + su."
    },
    {
        "question": "Karstik şekillerin yaygın olduğu ana kaya hangisidir?",
        "options": [
            "A) Granit",
            "B) Kalker (kireçtaşı)",
            "C) Bazalt",
            "D) Kumtaşı yalnız",
            "E) Kil"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Kalker çözünmesi → karst."
    },
    {
        "question": "Pamukkale hangi karstik/kimyasal birikim şekliyle anılır?",
        "options": [
            "A) Obruk",
            "B) Traverten",
            "C) Lapya",
            "D) Doline",
            "E) Polye"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Pamukkale = traverten."
    },
    {
        "question": "Türkiye kıyılarında fiyort tipinin görülmemesinin nedeni hangisidir?",
        "options": [
            "A) Ege kırıklıdır",
            "B) Buzul aşındırmasıyla oluşmuş derin körfezler (fiyort) Türkiye’de yoktur",
            "C) Karadeniz’de delta çoktur",
            "D) Akdeniz’de falez yoktur",
            "E) Marmara kapalıdır"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Fiyort = buzul kökenli; TR’de klasik fiyort yok."
    },
    {
        "question": "Ege kıyılarının girintili–çıkıntılı ve limanlaşmaya elverişli olmasının temel nedeni hangisidir?",
        "options": [
            "A) Dağların kıyıya paralel uzanması",
            "B) Horst–graben (kırıklı) yapı; dağların kıyıya dik/uzanım etkisi",
            "C) Buzullaşma",
            "D) Yalnızca rüzgâr",
            "E) Karstik obruklar"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ege kırıklı kıyı / graben körfezleri."
    },
    {
        "question": "Bir kıyıda delta ovası varsa hangisi kesin söylenemez?",
        "options": [
            "A) Alüvyon birikmiştir",
            "B) Kıyı görece sığ/birikim alanıdır",
            "C) Aynı yerde dik falez de vardır",
            "D) Akarsu denize ulaşmıştır",
            "E) Biriktirme olmuştur"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Delta + falez bir arada olmaz."
    },
    {
        "question": "İzohipsler birbirine yaklaştıkça ne artar?",
        "options": [
            "A) Sıcaklık",
            "B) Eğim",
            "C) Yağış her zaman",
            "D) Nüfus",
            "E) Nem"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sık izohips = dik/eğimli yer."
    },
    {
        "question": "Haritada bir noktanın yükseltisi nasıl okunur?",
        "options": [
            "A) Boylamdan",
            "B) İzohips değerinden",
            "C) Ölçekten yalnız",
            "D) Lejandan yalnız renk adından",
            "E) Kuzey okundan"
        ],
        "correctAnswerIndex": 1,
        "explanation": "İzohips = eşit yükselti eğrisi."
    },
    {
        "question": "Türkiye’de Ocak ayında en soğuk yerler genelde hangisidir?",
        "options": [
            "A) Antalya–Mersin",
            "B) Erzurum–Kars (kuzeydoğu)",
            "C) İzmir–Aydın",
            "D) Adana–Hatay",
            "E) Zonguldak–Bartın"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Klasik: Ocak en soğuk Erzurum–Kars."
    },
    {
        "question": "Gerçek sıcaklık ile indirgenmiş sıcaklık farkı neyi gösterir?",
        "options": [
            "A) Yalnızca enlemi",
            "B) Yükseltinin sıcaklık üzerindeki etkisini (deniz seviyesine indirgeme)",
            "C) Boylamı",
            "D) Yağış rejimini",
            "E) Rüzgâr yönünü"
        ],
        "correctAnswerIndex": 1,
        "explanation": "İndirgeme = yükselti etkisini ayıklamak."
    },
    {
        "question": "Bağıl nemin en yüksek olduğu bölge genelde hangisidir?",
        "options": [
            "A) İç Anadolu",
            "B) Karadeniz",
            "C) Güneydoğu Anadolu",
            "D) Doğu Anadolu’nun tamamı",
            "E) Trakya’nın içi"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Bağıl nem (yağış ihtimali) Karadeniz’de yüksektir."
    },
    {
        "question": "Mutlak nemin deniz kıyılarında (özellikle Akdeniz) yüksek olmasının nedeni hangisidir?",
        "options": [
            "A) Sıcak havanın daha fazla nem tutabilmesi",
            "B) Yükseltinin fazla olması",
            "C) Karasallık",
            "D) Buzul",
            "E) Fay"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Sıcak hava → mutlak nem kapasitesi yüksek."
    },
    {
        "question": "Akdeniz ikliminin Karadeniz ikliminden ayırt edici özelliği hangisidir?",
        "options": [
            "A) Her mevsim yağış",
            "B) Belirgin yaz kuraklığı",
            "C) Yazların serin ve yağışlı olması",
            "D) Yıllık sıcaklık farkının çok az olması",
            "E) Föhnün hiç görülmemesi"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Akdeniz = yaz kurak; Karadeniz’de yazlar da yağışlı."
    },
    {
        "question": "Sert karasal (Erzurum–Kars) ikliminin görüldüğü iller notlara göre hangileridir?",
        "options": [
            "A) İzmir–Manisa–Aydın",
            "B) Erzurum–Kars–Ardahan",
            "C) Antalya–Muğla–Mersin",
            "D) Edirne–Tekirdağ–Kırklareli",
            "E) Gaziantep–Kilis–Adıyaman"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sadece Erzurum, Kars, Ardahan."
    },
    {
        "question": "Yamaç yağışlarının belirgin olduğu bölgeler hangileridir?",
        "options": [
            "A) Yalnızca İç Anadolu",
            "B) Dağların kıyıya paralel uzandığı Karadeniz ve Akdeniz",
            "C) Yalnızca GAP",
            "D) Yalnızca Trakya",
            "E) Çöller"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Kıyıya paralel dağlar → yamaç yağışı."
    },
    {
        "question": "Aşağıdaki iklim–bitki/ürün bağlarından hangisi doğrudur?",
        "options": [
            "A) Akdeniz’de çay",
            "B) Karadeniz’de muz (geniş alan)",
            "C) Akdeniz’de muz (mikro klima / Anamur)",
            "D) İç Anadolu’da çay",
            "E) Erzurum’da zeytin"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Muz Akdeniz mikro klimasında; çay Doğu Karadeniz."
    },
    {
        "question": "Türkiye’de ormanların en yoğun olduğu bölge hangisidir?",
        "options": [
            "A) İç Anadolu",
            "B) Güneydoğu Anadolu",
            "C) Karadeniz",
            "D) Doğu Anadolu’nun tamamı",
            "E) Trakya içi"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Nem + yağış → Karadeniz ormanları."
    },
    {
        "question": "Türkiye akarsularında rejim düzensizliğinin temel nedeni hangisidir?",
        "options": [
            "A) Yağışın mevsimsel düzensizliği ve kar erimesi etkisi",
            "B) Boylam",
            "C) Saat dilimi",
            "D) Harita ölçeği",
            "E) FIR hattı"
        ],
        "correctAnswerIndex": 0,
        "explanation": "İklim + kar erimesi → düzensiz rejim."
    },
    {
        "question": "Türkiye’de can ve mal kaybı açısından en yıkıcı doğal afet genelde hangisidir?",
        "options": [
            "A) Çığ",
            "B) Deprem",
            "C) Dolu",
            "D) Sis",
            "E) Kırç"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Deprem = en yıkıcı."
    },
    {
        "question": "Heyelan riskinin yüksek olduğu bölge hangisidir?",
        "options": [
            "A) İç Anadolu’nun kurak ovaları",
            "B) Doğu Karadeniz (eğim + yağış + kil)",
            "C) GAP’ın düz ovaları",
            "D) Çatalca Platosu",
            "E) Ergene"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Doğu Karadeniz heyelan klasiktir."
    },
    {
        "question": "Cumhuriyet’in ilk nüfus sayımı hangi yıldadır?",
        "options": [
            "A) 1831",
            "B) 1927",
            "C) 1935",
            "D) 1950",
            "E) 1960"
        ],
        "correctAnswerIndex": 1,
        "explanation": "1927; önceki sayım olmadığı için artış hızı hesaplanamaz."
    },
    {
        "question": "Doğal nüfus artışının yüksek olduğu yerler genelde hangisidir?",
        "options": [
            "A) Gelişmiş batı metropolleri",
            "B) Doğu ve Güneydoğu (doğum fazla)",
            "C) Yalnızca kıyı turizm alanları",
            "D) Yalnızca sanayi bölgeleri",
            "E) Yalnızca plato alanları"
        ],
        "correctAnswerIndex": 1,
        "explanation": "3D = doğal artış → Doğu’da fazla."
    },
    {
        "question": "Gerçek nüfus artışının yüksek olması genelde neyi gösterir?",
        "options": [
            "A) Yalnızca doğum fazlalığı",
            "B) Göç alma (gelişmişlik / çekicilik)",
            "C) Yalnızca ölüm azlığı",
            "D) Yalnızca kır nüfusu",
            "E) Yalnızca yaşlı nüfus"
        ],
        "correctAnswerIndex": 1,
        "explanation": "3G = gerçek artış → göç alır."
    },
    {
        "question": "Türkiye’de nüfusun en yoğun olduğu kuşak hangisidir?",
        "options": [
            "A) Doğu Anadolu’nun tamamı",
            "B) İstanbul–Bursa ve batı kıyı/sanayi kuşakları",
            "C) Yalnızca Erzurum–Kars",
            "D) Yalnızca İç Anadolu stepı",
            "E) Yalnızca Toroslar"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sanayi–ticaret–ulaşım → batı yoğunluğu."
    },
    {
        "question": "Yaş bağımlılık oranında paydada yer alan grup hangisidir?",
        "options": [
            "A) 0–14 + 65+",
            "B) 15–64 (çalışma çağındaki nüfus)",
            "C) Yalnızca 65+",
            "D) Yalnızca 0–14",
            "E) Tüm nüfus"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Bağımlılık = (0–14+65+) / 15–64."
    },
    {
        "question": "Ortanca yaşın yükselmesi ne anlama gelir?",
        "options": [
            "A) Nüfus gençleşiyor",
            "B) Doğumlar azalıyor / yaşam süresi uzuyor; nüfus yaşlanıyor",
            "C) Göç duruyor",
            "D) Enlem değişiyor",
            "E) Yağış artıyor"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ortanca yaş ↑ = yaşlanma eğilimi."
    },
    {
        "question": "Kırdan kente göçün sonuçları arasında hangisi vardır?",
        "options": [
            "A) Kırsalda nüfus artar",
            "B) Kentlerde gecekondu/altyapı baskısı artabilir",
            "C) Tarım işgücü artar",
            "D) Doğumlar her yerde artar",
            "E) Sanayi geriler"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Kentlere baskı + kırda işgücü azalması."
    },
    {
        "question": "Türkiye’de kırsal yerleşme tipinin dağınık olduğu yerler genelde hangisidir?",
        "options": [
            "A) İç Anadolu düzlükleri",
            "B) Doğu Karadeniz (eğim, yağış, arazi parçalılığı)",
            "C) GAP ovalarının tamamı",
            "D) Ergene Ovası",
            "E) Konya Ovası"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Doğu Karadeniz = dağınık yerleşme."
    },
    {
        "question": "Toplu (küme) kırsal yerleşme hangi koşullarda yaygındır?",
        "options": [
            "A) Su kaynaklarının sınırlı olduğu düz/az engebeli alanlarda",
            "B) Her zaman dağlık ormanlarda",
            "C) Yalnızca falez kıyılarında",
            "D) Yalnızca buzul vadilerinde",
            "E) Yalnızca fiyortlarda"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Su + düz arazi → toplu yerleşme."
    },
    {
        "question": "1923 İzmir İktisat Kongresi’nin ruhu hangisine yakındır?",
        "options": [
            "A) Tam devletçilik",
            "B) Özel girişim ve millî ekonomi hedefleri",
            "C) Yalnızca tarım kapanması",
            "D) Yalnızca petrol ithali",
            "E) Yalnızca turizm"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Erken dönem: millî burjuvazi / özel teşebbüs vurgusu."
    },
    {
        "question": "1930’larda uygulanan ekonomi politikası hangisidir?",
        "options": [
            "A) Liberalizm yalnız",
            "B) Devletçilik",
            "C) Tam serbest piyasa",
            "D) Yalnızca ithalat",
            "E) Yalnızca hayvancılık"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Dünya buhranı sonrası devletçilik."
    },
    {
        "question": "Ekstansif tarımın temel özelliği hangisidir?",
        "options": [
            "A) Birim alandan yüksek verim + yoğun girdi",
            "B) Geniş alan, düşük girdi/verim; nadas yaygın olabilir",
            "C) Yalnızca sera",
            "D) Yalnızca organik",
            "E) Yalnızca çay"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Ekstansif = geniş alan / düşük yoğunluk."
    },
    {
        "question": "Nadasın azalmasına katkı sağlayan projeler notlara göre hangileridir?",
        "options": [
            "A) Yalnızca turizm",
            "B) GAP ve KOP (sulama)",
            "C) Yalnızca HES",
            "D) Yalnızca maden",
            "E) Yalnızca liman"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Sulama → nadas azalır."
    },
    {
        "question": "Çay üretiminin yoğunlaştığı yer hangisidir?",
        "options": [
            "A) İç Anadolu",
            "B) Doğu Karadeniz kıyı kuşağı",
            "C) GAP’ın tamamı",
            "D) Ege grabenleri",
            "E) Ergene"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Çay = Doğu Karadeniz (Rize vb.)."
    },
    {
        "question": "Fındık üretiminde öne çıkan bölge hangisidir?",
        "options": [
            "A) Akdeniz",
            "B) Karadeniz (özellikle Orta–Doğu)",
            "C) Güneydoğu",
            "D) İç Anadolu",
            "E) Doğu Anadolu yüksek yayla"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Fındık = Karadeniz."
    },
    {
        "question": "Pamuk üretiminin yoğun olduğu alanlara örnek hangisidir?",
        "options": [
            "A) Erzurum–Kars",
            "B) Çukurova, Ege grabenleri, GAP sulama alanları",
            "C) Doğu Karadeniz",
            "D) Kaçkar etekleri",
            "E) Buzul vadileri"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Pamuk sıcak + sulama ister."
    },
    {
        "question": "Zeytin için hangisi doğrudur?",
        "options": [
            "A) Donlu karasal yaylalarda yaygındır",
            "B) Akdeniz iklimi etkisi altındaki kıyılarda (özellikle Ege–Akdeniz–Güney Marmara) yetişir",
            "C) Yalnızca Doğu Anadolu’da",
            "D) Yalnızca İç Anadolu’da",
            "E) Yalnızca GAP’ın en doğusunda"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Zeytin = Akdeniz iklim kuşağı."
    },
    {
        "question": "Kayısı ile özdeşleşen il hangisidir?",
        "options": [
            "A) Rize",
            "B) Malatya",
            "C) Zonguldak",
            "D) Trabzon",
            "E) Antalya"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Malatya kayısı."
    },
    {
        "question": "Antep fıstığı üretiminde öne çıkan bölge hangisidir?",
        "options": [
            "A) Doğu Karadeniz",
            "B) Güneydoğu Anadolu (Gaziantep–Şanlıurfa çevresi)",
            "C) Marmara’nın tamamı",
            "D) Trakya",
            "E) Karadeniz yaylaları"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Antep fıstığı = Güneydoğu."
    },
    {
        "question": "Aşağıdaki ürün–bölge eşleştirmelerinden hangisi yanlıştır?",
        "options": [
            "A) Çay – Doğu Karadeniz",
            "B) Muz – Anamur (Akdeniz)",
            "C) Fındık – Karadeniz",
            "D) Pamuk – Erzurum–Kars",
            "E) Zeytin – Ege"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Erzurum–Kars pamuk için uygun değildir."
    },
    {
        "question": "I. Sulama artarsa nadas azalır\nII. GAP yalnızca enerji projesidir\nIII. Çukurova’da pamuk ve turunçgil önemlidir\nYargılarından hangileri doğrudur?",
        "options": [
            "A) Yalnız I",
            "B) I ve III",
            "C) II ve III",
            "D) Yalnız II",
            "E) I, II ve III"
        ],
        "correctAnswerIndex": 1,
        "explanation": "GAP çok sektörlüdür (tarım–enerji–ulaşım…); II yanlış."
    },
    {
        "question": "Küçükbaş hayvancılığın (kırkım/otlak) daha yaygın olduğu yerler hangisidir?",
        "options": [
            "A) Doğu Karadeniz orman içi",
            "B) İç Anadolu ve Doğu Anadolu’nun step/yayla alanları",
            "C) Yalnızca Marmara sanayi kuşağı",
            "D) Yalnızca Çukurova",
            "E) Yalnızca Zonguldak"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Step + yayla → koyun/keçi."
    },
    {
        "question": "Büyükbaş (süt) hayvancılığının modern işletmelerle öne çıktığı bölgeler genelde hangisidir?",
        "options": [
            "A) Yalnızca çöller",
            "B) Marmara, Ege ve İç Anadolu’nun gelişmiş tarım alanları",
            "C) Yalnızca falez kıyıları",
            "D) Yalnızca obruklar",
            "E) Yalnızca buzul sirkleri"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Pazar + yem + tesis → batı/gelişmiş alanlar."
    },
    {
        "question": "Türkiye’de maden çeşitliliğinin en fazla olduğu yer notlara göre hangisidir?",
        "options": [
            "A) Çatalca Platosu",
            "B) Yukarı Fırat (Elazığ çevresi)",
            "C) Ergene Ovası",
            "D) Bafra Deltası",
            "E) İstanbul Boğazı"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Yukarı Fırat / Elazığ — volkanizma etkisi."
    },
    {
        "question": "Bor madeninin başlıca çıkarım/işleme alanları hangileridir?",
        "options": [
            "A) Zonguldak–Bartın",
            "B) Eskişehir–Kütahya–Balıkesir (Kırka, Bandırma)",
            "C) Rize–Artvin",
            "D) Antalya–Mersin",
            "E) Erzurum–Kars"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Bor = Eskişehir vb.; Kırka–Bandırma."
    },
    {
        "question": "Krom çıkarımında öne çıkan yerler hangileridir?",
        "options": [
            "A) Zonguldak",
            "B) Elazığ (Guleman) ve Fethiye (Köyceğiz)",
            "C) Batman",
            "D) Sinop",
            "E) Edirne"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Krom: Guleman–Köyceğiz; işleme Elazığ/Antalya."
    },
    {
        "question": "Bakır çıkarım yerlerine örnek hangisidir?",
        "options": [
            "A) Kastamonu (Küre), Artvin (Murgul), Rize (Çayeli)",
            "B) Antalya muz alanları",
            "C) Konya Ovası",
            "D) Ergene",
            "E) Bafra"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Küre–Murgul–Çayeli."
    },
    {
        "question": "Taş kömürü çıkarımıyla özdeşleşen il hangisidir?",
        "options": [
            "A) Batman",
            "B) Zonguldak (Ereğli)",
            "C) Antalya",
            "D) Muğla",
            "E) Şanlıurfa"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Zonguldak taş kömürü."
    },
    {
        "question": "Aşağıdaki maden–yer eşleştirmelerinden hangisi yanlıştır?",
        "options": [
            "A) Bor – Eskişehir",
            "B) Krom – Elazığ",
            "C) Taş kömürü – Zonguldak",
            "D) Bor – Zonguldak",
            "E) Bakır – Murgul"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Zonguldak taş kömürü; bor değil."
    },
    {
        "question": "Türkiye’de taş kömürünün başlıca bulunduğu yerler hangileridir?",
        "options": [
            "A) Batman–Siirt",
            "B) Zonguldak–Bartın (Kozlu, Karadon, Amasra)",
            "C) Antalya–Mersin",
            "D) Konya–Karaman",
            "E) Edirne–Kırklareli"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Taş kömürü = Zonguldak havzası."
    },
    {
        "question": "Türkiye’nin en büyük linyit santrali notlara göre hangisidir?",
        "options": [
            "A) Soma yalnız",
            "B) Afşin–Elbistan (Kahramanmaraş)",
            "C) Çatalağzı yalnız",
            "D) Aliağa",
            "E) Akkuyu"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Afşin–Elbistan."
    },
    {
        "question": "Türkiye’de ilk petrolün bulunduğu yer hangisidir?",
        "options": [
            "A) Zonguldak",
            "B) Batman",
            "C) İstanbul",
            "D) İzmir",
            "E) Ankara"
        ],
        "correctAnswerIndex": 1,
        "explanation": "İlk petrol Batman."
    },
    {
        "question": "Türkiye’nin petrol ihtiyacının büyük bölümü için hangisi doğrudur?",
        "options": [
            "A) Tamamen yerli üretim yeter",
            "B) Yaklaşık %90’ı ithal edilir",
            "C) İhracat fazladır",
            "D) Hiç petrol kullanılmaz",
            "E) Yalnızca linyitten üretilir"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Yüksek ithalat bağımlılığı."
    },
    {
        "question": "Doğal gazda dışa bağımlılık için notlara göre hangisi doğrudur?",
        "options": [
            "A) Tamamen yerli",
            "B) İhtiyacın çok büyük bölümü ithal edilir",
            "C) İhracatçıdır",
            "D) Yalnızca kömürden üretilir",
            "E) Yalnızca jeotermaldir"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Doğal gazda bağımlılık çok yüksektir."
    },
    {
        "question": "Türkiye’nin ilk nükleer güç santrali projesi nerededir?",
        "options": [
            "A) Zonguldak",
            "B) Mersin (Akkuyu)",
            "C) Erzurum",
            "D) Van",
            "E) Sinop işletmede"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Akkuyu – Mersin (inşaat süreci)."
    },
    {
        "question": "Yenilenebilir enerji kaynaklarına örnek hangisidir?",
        "options": [
            "A) Linyit",
            "B) Taş kömürü",
            "C) Hidroelektrik / rüzgâr / güneş / jeotermal",
            "D) Petrol",
            "E) Asfaltit"
        ],
        "correctAnswerIndex": 2,
        "explanation": "HES–rüzgâr–güneş–jeotermal yenilenebilir."
    },
    {
        "question": "Türkiye’de sanayinin en yoğun olduğu bölge hangisidir?",
        "options": [
            "A) Doğu Anadolu",
            "B) Marmara",
            "C) Güneydoğu’nun tamamı",
            "D) Karadeniz yaylaları",
            "E) İç Anadolu stepı yalnız"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Marmara = sanayi kalbi."
    },
    {
        "question": "Ham maddeye bağlı sanayi yerleşimine örnek hangisidir?",
        "options": [
            "A) İstanbul tekstili yalnız",
            "B) Şeker fabrikalarının pancar alanlarına yakınlığı",
            "C) Yalnızca yazılım",
            "D) Yalnızca turizm",
            "E) Yalnızca liman gümrüğü"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Şeker = ham maddeye yakın."
    },
    {
        "question": "Türkiye’de en çok kullanılan yük/yolcu taşıma sistemi hangisidir?",
        "options": [
            "A) Denizyolu yalnız",
            "B) Karayolu",
            "C) Boru hattı yalnız",
            "D) Hava yalnız",
            "E) Nehir"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Karayolu baskındır."
    },
    {
        "question": "Boğaz köprüleri ve geçitler hangi coğrafi engeli aşmaya yöneliktir?",
        "options": [
            "A) Çöl",
            "B) Su engeli / boğaz geçişi",
            "C) Enlem",
            "D) Boylam",
            "E) İzohips"
        ],
        "correctAnswerIndex": 1,
        "explanation": "İstanbul/Çanakkale boğaz geçişleri."
    },
    {
        "question": "Deniz–kum–güneş turizminin yoğun olduğu bölgeler hangileridir?",
        "options": [
            "A) Doğu Anadolu yüksek yaylaları",
            "B) Akdeniz ve Ege kıyıları",
            "C) Erzurum–Kars kış turizmi yalnız",
            "D) Zonguldak maden sahaları",
            "E) İç Anadolu stepı"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Akdeniz–Ege kıyı turizmi."
    },
    {
        "question": "Kış turizmi (kayak) merkezlerine örnek hangisidir?",
        "options": [
            "A) Anamur muz alanları",
            "B) Uludağ, Palandöken, Kartalkaya, Erciyes",
            "C) Çukurova",
            "D) Bafra Deltası",
            "E) Ergene"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Kayak = yüksek dağ merkezleri."
    },
    {
        "question": "Ege’de Türkiye–Yunanistan sorunları arasında hangisi yer almaz?",
        "options": [
            "A) Kıta sahanlığı",
            "B) Karasuları",
            "C) FIR hattı",
            "D) SAR bölgesi",
            "E) Fırat’ın kullanımı"
        ],
        "correctAnswerIndex": 4,
        "explanation": "Fırat = Suriye/Irak su sorunu; Ege değil."
    },
    {
        "question": "Türkiye’nin Avrupa’ya giden en işlek sınır kapısı hangisidir?",
        "options": [
            "A) Habur",
            "B) Kapıkule",
            "C) Sarp",
            "D) Gürbulak",
            "E) Dilucu"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Kapıkule."
    },
    {
        "question": "Habur sınır kapısı hangi ülkeyledir?",
        "options": [
            "A) İran",
            "B) Irak",
            "C) Gürcistan",
            "D) Bulgaristan",
            "E) Yunanistan"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Habur = Irak; İran = Gürbulak."
    },
    {
        "question": "GAP’ın kapsamı için hangisi doğrudur?",
        "options": [
            "A) Yalnızca turizm",
            "B) Tarım, enerji, ulaşım, sanayi gibi çok sektörlü kalkınma",
            "C) Yalnızca maden",
            "D) Yalnızca balıkçılık",
            "E) Yalnızca orman"
        ],
        "correctAnswerIndex": 1,
        "explanation": "GAP çok amaçlıdır."
    },
    {
        "question": "Aşağıdaki eşleştirmelerden hangisi yanlıştır?",
        "options": [
            "A) Bakı – yamaç yönü etkisi",
            "B) Enlem – K–G sıcaklık",
            "C) İndirgenmiş sıcaklık – yükselti etkisini ayıklama",
            "D) Fiyort – Türkiye’nin Ege kıyılarında yaygındır",
            "E) Horst – kırılma ile yüksekte kalan kütle"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Fiyort TR’de yok; Ege kırıklı/ria karakterlidir."
    },
    {
        "question": "I. Karadeniz’de yazlar da yağışlıdır\nII. Akdeniz’de yaz kuraklığı belirgindir\nIII. Erzurum–Kars’ta Ocak en soğuk merkezlerdendir\nHangileri doğrudur?",
        "options": [
            "A) Yalnız I",
            "B) I ve II",
            "C) II ve III",
            "D) I ve III",
            "E) I, II ve III"
        ],
        "correctAnswerIndex": 4,
        "explanation": "Üçü de doğru."
    },
    {
        "question": "Aşağıdakilerden hangisi “bakı = her zaman güney yamaç daha sıcaktır” yargısını çürütür?",
        "options": [
            "A) Ege’de zeytin",
            "B) Karadeniz’de kışın kuzey yamaçların daha ılık olabilmesi",
            "C) Akdeniz’de muz",
            "D) İç Anadolu’da step",
            "E) GAP’ta pamuk"
        ],
        "correctAnswerIndex": 1,
        "explanation": "Karadeniz ters olay / bakı çeldiricisi."
    },
    {
        "question": "Bölge–ürün–şekil bağlarından hangisi doğrudur?",
        "options": [
            "A) Doğu Karadeniz – çay – dağınık yerleşme",
            "B) İç Anadolu – çay – fiyort",
            "C) Ege – bor – buzul sirk",
            "D) Zonguldak – pamuk – delta",
            "E) Erzurum – zeytin – falez"
        ],
        "correctAnswerIndex": 0,
        "explanation": "Çay + dağınık yerleşme = Doğu Karadeniz."
    },
    {
        "question": "Aşağıdaki yargılardan hangisi yanlıştır?",
        "options": [
            "A) Ege’de horst–graben yaygındır",
            "B) KAF ve DAF Karlıova’da kesişir",
            "C) Delta olan yerde falez olmaz",
            "D) Türkiye’de fiyort kıyıları geniştir",
            "E) Bor Eskişehir çevresinde yoğundur"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Fiyort TR’de yoktur."
    },
    {
        "question": "I. Afşin–Elbistan linyit santralidir\nII. Akkuyu nükleer santral projesidir\nIII. Batman ilk petrol sahasıdır\nHangileri doğrudur?",
        "options": [
            "A) Yalnız I",
            "B) I ve II",
            "C) II ve III",
            "D) I ve III",
            "E) I, II ve III"
        ],
        "correctAnswerIndex": 4,
        "explanation": "Hepsi doğru."
    },
    {
        "question": "Aşağıdaki neden–sonuç ilişkilerinden hangisi yanlıştır?",
        "options": [
            "A) Yükselti artar → sıcaklık düşer",
            "B) Sulama artar → nadas azalır",
            "C) Dağlar kıyıya paralel → iç kesimlere nemli hava zor girer",
            "D) Enlem artar (kuzeye) → genelde sıcaklık artar",
            "E) Göç alır → gerçek nüfus artışı yükselir"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Kuzeye gidildikçe (enlem ↑) sıcaklık genelde düşer."
    },
    {
        "question": "Maden–enerji tuzaklarından hangisi doğrudur?",
        "options": [
            "A) Bor Zonguldak’tadır",
            "B) Taş kömürü Eskişehir’dedir",
            "C) Krom Elazığ–Fethiye’dedir",
            "D) Petrol ilk kez Rize’de bulunmuştur",
            "E) Doğal gaz tamamen yerlidir"
        ],
        "correctAnswerIndex": 2,
        "explanation": "Krom Guleman–Köyceğiz."
    },
    {
        "question": "Aşağıdakilerden hangisi GAP illeri arasında gösterilemez?",
        "options": [
            "A) Şanlıurfa",
            "B) Diyarbakır",
            "C) Gaziantep",
            "D) Trabzon",
            "E) Mardin"
        ],
        "correctAnswerIndex": 3,
        "explanation": "Trabzon Karadeniz’dir; GAP’ta değildir."
    }
];
