// sorular/turkce-3.js - Paragraf
window.turkce_3_sorulari = [
{
    "question": "Okuyucuyu bilgilendirmeyi amaçlayan anlatım türü hangisidir?",
    "options": [
        "A) Tartışmacı",
        "B) Betimleyici",
        "C) Öyküleyici",
        "D) Açıklayıcı"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Açıklayıcı anlatım öğretmeyi / bilgi vermeyi amaçlar."
},
{
    "question": "Bir düşüncenin yanlışlığını gösterip başka bir düşünceyi savunan anlatım hangisidir?",
    "options": [
        "A) Açıklayıcı",
        "B) Betimleyici",
        "C) Tartışmacı",
        "D) Öyküleyici"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Tartışmacı anlatımda sav ve çürütme vardır."
},
{
    "question": "Okuyucunun zihninde görüntü oluşturmaya yönelik anlatım hangisidir?",
    "options": [
        "A) Betimleyici",
        "B) Açıklayıcı",
        "C) Tartışmacı",
        "D) Öyküleyici"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Betimleme gözlemleri canlandırır."
},
{
    "question": "Belirli yer ve zamana bağlı olayların anlatıldığı anlatım hangisidir?",
    "options": [
        "A) Açıklayıcı",
        "B) Öyküleyici",
        "C) Tartışmacı",
        "D) Betimleyici"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Öyküleme olay, kahraman, yer ve zaman ister."
},
{
    "question": "“Korku; gerçek bir tehlikenin uyandırdığı kaygı duygusudur.” cümlesinde hangi düşünceyi geliştirme yolu vardır?",
    "options": [
        "A) Örnekleme",
        "B) Tanık gösterme",
        "C) Benzetme",
        "D) Tanımlama"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Korkunun ne olduğu tanımlanmıştır."
},
{
    "question": "İki varlığın benzer veya farklı yönlerinin kıyaslanmasına ne denir?",
    "options": [
        "A) Karşılaştırma",
        "B) Örneklendirme",
        "C) Tanık gösterme",
        "D) Açıklama"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Kıyas karşılaştırma yoludur."
},
{
    "question": "“Mesela bir Türk ‘hayır’ anlamında başını yukarı kaldırır.” ifadesinde hangi yöntem vardır?",
    "options": [
        "A) Benzetme",
        "B) Örneklendirme",
        "C) Tanımlama",
        "D) Tanık gösterme"
    ],
    "correctAnswerIndex": 1,
    "explanation": "“Mesela” örneklemeyi işaret eder."
},
{
    "question": "“Birikimsiz yazarlık saman alevi gibidir.” cümlesinde hangi yöntem kullanılmıştır?",
    "options": [
        "A) Karşılaştırma",
        "B) Benzetme",
        "C) Sayısal veri",
        "D) Tanımlama"
    ],
    "correctAnswerIndex": 1,
    "explanation": "“gibidir” benzetme ilgecidir."
},
{
    "question": "Bir düşünceyi desteklemek amacıyla uzman kişinin görüşüne yer verilmesine ne denir?",
    "options": [
        "A) Örneklendirme",
        "B) Betimleme",
        "C) Karşılaştırma",
        "D) Tanık gösterme"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Otorite görüşü tanık göstermedir."
},
{
    "question": "“Türkiye’nin nüfusu 79 milyon 814 bin 871 olmuştur.” ifadesinde hangi yöntem vardır?",
    "options": [
        "A) Sayısal verilerden yararlanma",
        "B) Tanımlama",
        "C) Benzetme",
        "D) Tanık gösterme"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Kesin sayı, sayısal veridir."
},
{
    "question": "Paragrafın açıklanıp geliştirilebilecek genel ve bağımsız yargılarının bulunduğu bölüm hangisidir?",
    "options": [
        "A) Giriş",
        "B) Gelişme",
        "C) Sonuç",
        "D) Ara bölüm"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Giriş cümlesi bağımsız ve geneldir."
},
{
    "question": "Konunun örneklerle zenginleştirilip açıklandığı bölüm hangisidir?",
    "options": [
        "A) Giriş",
        "B) Sonuç",
        "C) Gelişme",
        "D) Başlık"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Gelişme bölümü konuyu açar."
},
{
    "question": "Konunun özetlenip sonuca bağlandığı bölüm hangisidir?",
    "options": [
        "A) Giriş",
        "B) Gelişme",
        "C) Sonuç",
        "D) Yardımcı düşünce"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Sonuç özetler ve bağlar."
},
{
    "question": "Paragraftaki anlam bütünlüğünü bozan cümleye ne ad verilir?",
    "options": [
        "A) Ana düşünce",
        "B) Sonuç",
        "C) Yardımcı düşünce",
        "D) Akışı bozan cümle"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Konu dışına çıkan cümle akışı bozar."
},
{
    "question": "Paragraf ikiye bölünürken temel olarak neye dikkat edilir?",
    "options": [
        "A) Cümle uzunluğuna",
        "B) Noktalama işaretlerine",
        "C) Ana düşüncenin veya yönün değişmesine",
        "D) Sözcük sayısına"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Yön veya ana düşünce değişince iki paragraf olur."
},
{
    "question": "“Yazar ne anlatıyor?” sorusu paragrafta hangi unsuru buldurur?",
    "options": [
        "A) Ana düşünce",
        "B) Konu",
        "C) Başlık",
        "D) Yardımcı düşünce"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Ne anlatıldığı konudur."
},
{
    "question": "Paragrafın konusunun bir veya birkaç sözcükle ifade edilmesine ne denir?",
    "options": [
        "A) Sonuç",
        "B) Ana düşünce",
        "C) Yardımcı düşünce",
        "D) Başlık"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Başlık konuyu kısa adlandırır."
},
{
    "question": "Yazarın okuyucuya vermek istediği mesaja ne denir?",
    "options": [
        "A) Konu",
        "B) Başlık",
        "C) Ana düşünce",
        "D) Yardımcı düşünce"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Ana düşünce iletilmek istenen yargıdır."
},
{
    "question": "Ana düşünceyi destekleyen ve açıklayan düşüncelere ne denir?",
    "options": [
        "A) Yardımcı düşünce",
        "B) Başlık",
        "C) Konu",
        "D) Sonuç"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Yardımcı düşünceler ana fikri taşır."
},
{
    "question": "“Değinilmemiştir, çıkarılamaz, söylenemez” ifadeleri genellikle hangi tür sorularda kullanılır?",
    "options": [
        "A) Ana düşünce",
        "B) Yardımcı düşünce",
        "C) Başlık",
        "D) Konu"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Bu kalıplar yardımcı düşünce / çıkarım sorularına aittir."
}
];
