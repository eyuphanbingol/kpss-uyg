// sorular/cografya-10-soru.js
window.cografya_10_sorulari = [
  {
    "id": "1",
    "question": "Türkiye'de kıyı şekillenmesi üzerinde etkili olan faktörler arasında aşağıdakilerden hangisi GÖSTERİLEMEZ?",
    "options": [
      "A) Dalgalar",
      "B) Akarsular",
      "C) Rüzgârlar",
      "D) Dağların uzanışı ve epirojenez",
      "E) Volkanizma tek başına"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Kıyı şekillenmesinde en temel dış kuvvet dalgalar ve akıntılardır. Ayrıca akarsular, rüzgarlar, dağların uzanış doğrultusu ve epirojenez (kıyı çizgisinin değişmesi) doğrudan veya dolaylı etkilidir. Volkanizma tek başına kıyı şeklini belirleyen genel bir dış kıyı şekillendirici faktör değildir."
  },
  {
    "id": "2",
    "question": "Dalgaların dik yamaçlarda oluşturduğu aşındırma şekli ile sığ kıyılarda oluşturduğu biriktirme sürecine ilişkin genel kural aşağıdakilerden hangisinde doğru verilmiştir?",
    "options": [
      "A) Dik yamaçlarda biriktirme, sığ kıyılarda aşındırma olur",
      "B) Dik yamaçlarda aşındırma, sığ kıyılarda biriktirme olur",
      "C) Her ikisinde de yalnızca aşındırma olur",
      "D) Her ikisinde de yalnızca biriktirme olur",
      "E) Dalgalar yalnızca sığ kıyılarda etkilidir"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Dalga şekillendirmesinin genel kuralı şudur: Dağların dik indiği derin kıyılarda dalgalar çarparak aşındırma (falez) yapar, deniz derinliğinin az olduğu sığ kıyılarda ise enerjisini kaybederek taşıdığı malzemeyi biriktirir (plaj, kıyı oku)."
  },
  {
    "id": "3",
    "question": "Aşağıdaki dalga aşındırma şekli–bölge eşleştirmelerinden hangisi doğrudur?",
    "options": [
      "A) Falez (yalıyar) – Doğu ve Batı Karadeniz, Antalya-Yıldız Dağları kıyıları",
      "B) Falez – yalnızca Ege kıyıları",
      "C) Aşınım platosu – yalnızca Marmara kıyıları",
      "D) Falez – delta ovalarının bulunduğu her yer",
      "E) Aşınım platosu – yalnızca Doğu Anadolu"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Dağların kıyıya paralel ve yakın uzandığı Doğu Karadeniz, Batı Karadeniz, Antalya kıyıları ve Marmara Bölgesi'ndeki Yıldız Dağları kıyıları falez (yalıyar) oluşumunun en yaygın ve doğru eşleştirildiği alanlardır."
  },
  {
    "id": "4",
    "question": "Falezler ile deniz arasında kalan düzlüklere ne ad verilir ve deltanın bulunduğu yerde bu şekil neden görülmez?",
    "options": [
      "A) Aşınım platosu; çünkü deltalar biriktirme, falez ise aşındırma alanıdır",
      "B) Kıyı kordonu; çünkü deltalar çok derindir",
      "C) Lagün; çünkü deltalarda tuzluluk fazladır",
      "D) Tombolo; çünkü deltalar ada değildir",
      "E) Plaj; çünkü deltalarda kumsal olmaz"
    ],
    "correctAnswerIndex": 0,
    "explanation": "Falezlerin gerilemesiyle falez ile deniz arasında kalan düzlüklere aşınım platosu denir. Delta ovalarının olduğu yer sığ ve biriktirme alanı iken, falez derin ve dik yamaçlı aşındırma alanıdır; bu nedenle ikisi aynı yerde bulunamaz."
  },
  {
    "id": "5",
    "question": "Dalga biriktirme şekilleriyle ilgili;\nI. Kıyı oku tamamen kapanırsa kıyı kordonu oluşur.\nII. Kıyı kordonunun kapattığı su kütlesine lagün (deniz kulağı) denir.\nIII. Bir adanın karaya bağlanmış hâline tombolo (saplı ada) denir.\nyargılarından hangileri doğrudur?",
    "options": [
      "A) Yalnız I",
      "B) I ve II",
      "C) I ve III",
      "D) II ve III",
      "E) I, II ve III"
    ],
    "correctAnswerIndex": 4,
    "explanation": "Öncüllerde verilen tüm tanımlar dalga biriktirme şekillerine aittir ve hepsi coğrafi olarak tamamen doğrudur."
  },
  {
    "id": "6",
    "question": "Kapıdağ Yarımadası'nın (Balıkesir) örneği ile bilinen, bir adanın kıyı biriktirmesiyle karaya bağlanmasından oluşan şekil aşağıdakilerden hangisidir?",
    "options": [
      "A) Lagün",
      "B) Kıyı oku",
      "C) Tombolo (saplıada)",
      "D) Falez",
      "E) Aşınım platosu"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Kıyıya yakın bir adanın dalga biriktirmesi sonucu karaya bağlanarak yarımadaya dönüşmesi şekline tombolo (saplı ada) denir. Türkiye'de en belirgin örneği Balıkesir'deki Kapıdağ Yarımadası'dır."
  },
  {
    "id": "7",
    "question": "Finike ile Kaş arasında, Teke Yarımadası'nın güneyinde görülen kıyı tipi aşağıdakilerden hangisidir?",
    "options": [
      "A) Ria kıyısı",
      "B) Boyuna kıyı",
      "C) Kalanlı kıyı",
      "D) Dalmaçya kıyısı",
      "E) Falezli kıyı"
    ],
    "correctAnswerIndex": 3,
    "explanation": "Deniz seviyesinin yükselmesiyle kıyıya paralel uzanan dağlar arasındaki vadilerin sular altında kalması ve dağ zirvelerinin adacıklara dönüşmesiyle oluşan kıyı tipine Dalmaçya kıyı tipi denir. Türkiye'de sadece Antalya Finike-Kaş arasında görülür."
  },
  {
    "id": "8",
    "question": "Deniz seviyesinin yükselmesi veya kıyı kesimindeki toprakların çökmesiyle, kıyıya dik eski akarsu vadilerinin sular altında kalmasıyla oluşan; İstanbul-Çanakkale Boğazı ve Gökova Körfezi'nin örnek gösterildiği kıyı tipi aşağıdakilerden hangisidir?",
    "options": [
      "A) Dalmaçya kıyısı",
      "B) Ria kıyı",
      "C) Kalanlı kıyı",
      "D) Boyuna kıyı",
      "E) Falez kıyısı"
    ],
    "correctAnswerIndex": 1,
    "explanation": "Eski akarsu vadilerinin deniz suları altında kalmasıyla oluşan girintili çıkıntılı kıyı tipine Ria kıyı tipi denir. İstanbul Boğazı, Çanakkale Boğazı ve Muğla'daki Gökova Körfezi bu kıyı tipinin en net örnekleridir."
  },
  {
    "id": "9",
    "question": "Mersin-Silifke kıyılarında görülen, karstik akarsu vadilerinin denizle ilişkili olduğu kıyı tipi aşağıdakilerden hangisidir?",
    "options": [
      "A) Ria kıyı",
      "B) Dalmaçya kıyısı",
      "C) Kalanlı kıyı",
      "D) Boyuna kıyı",
      "E) Falezli kıyı"
    ],
    "correctAnswerIndex": 2,
    "explanation": "Karstik arazilerdeki akarsu vadilerinin veya kanyonların deniz suları altında kalmasıyla oluşan özel kıyı tipine Kalanlı kıyı tipi denir. Türkiye'de Mersin-Silifke kıyıları bu duruma örnektir."
  }
];