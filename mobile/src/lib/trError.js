...
function hasEnglishCue(errText) {
    return /\b(invalid|failed|unable|error|denied|missing|unauthorized|forbidden|network|timeout|expired|token|password|email|user|fetch|credentials|confirm|registered|request|please|success)\b/i.test(errText);
}
...
var RULES = [
    ...
    [/session missing|auth session missing|refresh_token|invalid_grant|jwt expired/i, "Oturum yok veya süresi doldu. Yeni bir sıfırlama maili iste."],
...
export function trError(err, fallback) {
